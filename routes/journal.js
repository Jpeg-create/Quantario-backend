...
router.get('/', (req, res) => {
  try { res.json({ success: true, data: dbAll('SELECT * FROM journal_entries WHERE user_id = ? ORDER BY entry_date DESC', [req.user.id]) }); }
...
router.post('/', (req, res) => {
  try {
    ...
    const id = uuidv4();
    dbRun('INSERT INTO journal_entries (id, user_id, entry_date, content) VALUES (?, ?, ?, ?)', [id, req.user.id, entry_date, content]);
...
router.delete('/:id', (req, res) => {
  try {
    const existing = dbGet('SELECT id FROM journal_entries WHERE id = ? AND user_id = ?', [req.params.id, req.user.id]);
